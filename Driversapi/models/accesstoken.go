...
func (accessToken *AccessToken) GetUser() Driver {
	driver.Password = ""
	return driver
}
...
func (accessToken *AccessToken) AuthoriseByToken(w http.ResponseWriter, r *http.Request) bool {
	...
	err = db.C("user").Find(bson.M{"_id": accessToken.UserID}).One(&driver)
	if err != nil {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
